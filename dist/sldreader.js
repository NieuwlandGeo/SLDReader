/* Version: 0.7.2 - August 13, 2025 11:03:40 */
...
class RadialShape extends ImageStyle {
  ...
  getImage(pixelRatio) {
    ...
    if (!image) {
      ...
      shared.set(cacheKey, null, null, new IconImage(image, undefined, null, ImageState.LOADED, null));
    }
    return image;
  }
...
function splitLineString(geometry, graphicSpacing) {
  ...
  while (nextPointMeasure <= totalLength) {
    ...
    if (cumulativeMeasure + currentSegmentLength < nextPointMeasure) {
      ...
      if (!splitOptions.extent || containsCoordinate(splitOptions.extent, splitPointCoords)) {
...
function getGraphicStrokeRenderer(linesymbolizer) {
  ...
  return (pixelCoords, renderState) => {
    ...
    renderStrokeMarks(render, pixelCoords, graphicSpacing, pointStyle, pixelRatio, options);
  };
}
...
export { OlStyler, Reader, categorizeSymbolizers, createOlStyle, createOlStyleFunction, getByPath, getFunction, getLayer, getLayerNames, getRuleSymbolizers, getRules, getStyle, getStyleNames, registerCustomSymbol, registerFunction, version, warnOnce };
